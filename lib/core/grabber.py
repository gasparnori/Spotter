        ...
        if self.capture_type == "opencv":
            ...
            if self.source_type == 'device':
                ...
                if self.size_init is not None:
                    if self.capture.get(cv2.CAP_PROP_FRAME_WIDTH)>self.size_init[0]:
                        self.log.debug("Setting frame size of capture: {0[0]}x{0[1]}".format(self.size_init))
                    self.capture.set(cv2.CAP_PROP_FRAME_WIDTH, float(self.size_init[0]))
                    if self.capture.get(cv2.CAP_PROP_FRAME_HEIGHT) > self.size_init[1]:
                        self.capture.set(cv2.CAP_PROP_FRAME_HEIGHT, float(self.size_init[1]))
